...
public class PermInd implements Ind {
    ...
    public List<Ind> recombinar(Ind outro) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            double I = cmax - cmin;
            double min = cmin - alpha * I;
            double max = cmax + alpha * I;
...
public List<Ind> recombinar(Ind outro) {
    ...
    double alpha = 0.5; // Fator de expansão do intervalo
    double[] filho = new double[n];
    ...
    for (int i = 0; i < n; i++) {
        ...
        filho[i] = min + (max - min) * rand.nextDouble();
        filho[i] = Math.max(LIMITE_INFERIOR, Math.min(LIMITE_SUPERIOR, filho[i]));
    }
    List<PermInd> filhos = new List<PermInd>();
filo
